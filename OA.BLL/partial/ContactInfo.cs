...
namespace CanYou.OA.BLL
{
	...
	/// </summary>
	[Serializable]
	public partial class ContactInfo:CanYou.EntityObject
	{
		#region 属性
		///<summary>
		///
		...
		///
		///</summary>
		private string phoneExt = String.Empty;
		///<summary>
		///
		///</summary>
		...
		///
		///</summary>
		public ContactInfo(int pContactId,string pMobile,string pOfficephone,string pPhoneExt,string pHomephone,string pEmail)
		{
			contactId   = pContactId;
			mobile      = pMobile;
			officephone = pOfficephone;
			phoneExt    = pPhoneExt;
		...
		{
			 LoadFromId(contactId);
		}
		
		private void LoadFromId(int contactId)
		{
		 
			if (CachedEntityCommander.IsTypeRegistered(typeof(ContactInfo)))
            {
				ContactInfo contactInfo=Find(GetList(), contactId);
				if(contactInfo==null)
					throw new AppException("未能在缓存中找到相应的键值对象");
                Copy(contactInfo, this);
            }
            else
            {	Contact contact=new Contact( contactId);	
				if(contact.IsNew)
				throw new AppException("尚未初始化");
               	LoadFromDAL(this, contact);
            }
		}
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static  ContactInfo Find(List< ContactInfo> list,  int contactId)
        {
             return list.Find(delegate( ContactInfo item) { return item. contactId==contactId; });
        }
        ...
        {
            return string.Format(CachedEntityCommander.CacheKey, typeof( ContactInfo).ToString());
        }
		...
		public  override void Delete()
		{
			if(!m_Loaded)
				throw new AppException("尚未初始化");
			bool result=  (Contact.Delete(ContactId) == 1);
			//更新缓存
			if (result&&CachedEntityCommander.IsTypeRegistered(typeof(ContactInfo))) 
            {
                ResetCache();
            }
            if(!result)
