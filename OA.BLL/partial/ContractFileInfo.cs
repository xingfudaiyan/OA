...
{
	/// <summary>
	///ContractFileInfo  Data
	/// </summary>
	[Serializable]
	public partial class ContractFileInfo:CanYou.EntityObject
	{
		#region 属性
		///<summary>
		///
		...
		///
		///</summary>
		private string contractFileName = String.Empty;
		///<summary>
		///
		///</summary>
		...
		public struct Fields
		{
			...
			public const string ContractFileName=@"ContractFileName";
		...
		///
		///</summary>
		public ContractFileInfo(int pContractFileId,string pContractFileName,string pPhyFileName)
		{
			contractFileId   = pContractFileId;
			contractFileName = pContractFileName;
		...
		///
		///</summary>
		public ContractFileInfo(int contractFileId)
		{
			 LoadFromId(contractFileId);
		}
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static  ContractFileInfo Find(List< ContractFileInfo> list,  int contractFileId)
        {
             return list.Find(delegate( ContractFileInfo item) { return item. contractFileId==contractFileId; });
        }
        ...
        {
            return string.Format(CachedEntityCommander.CacheKey, typeof( ContractFileInfo).ToString());
        }
		...
		{
			get {return contractFileName;}
			set {contractFileName = value;}
		}
		...
		public  override void Delete()
		{
			if(!m_Loaded)
				throw new AppException("尚未初始化");
			bool result=  (ContractFile.Delete(ContractFileId) == 1);
			//更新缓存
			if (result&&CachedEntityCommander.IsTypeRegistered(typeof(ContractFileInfo))) 
            {
                ResetCache();
            }
            if(!result)
		...
		public override void Save()
		{
			if(!m_Loaded)//新增
			{
				...
				SaveToDb(this, contractFile,true);
			}
			else//修改
			{
				ContractFile contractFile=new ContractFile(contractFileId);	
				...
				SaveToDb(this, contractFile,false);
			}
