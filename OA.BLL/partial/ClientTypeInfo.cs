...
namespace CanYou.OA.BLL
{
	...
	/// </summary>
	[Serializable]
	public partial class ClientTypeInfo:CanYou.EntityObject
	{
		#region 属性
		///<summary>
		///
		...
		///
		///</summary>
		public ClientTypeInfo(int pClientTypeId,string pClientTypeName)
		{
			clientTypeId   = pClientTypeId;
			clientTypeName = pClientTypeName;
		...
		///
		///</summary>
		public ClientTypeInfo(int clientTypeId)
		{
			 LoadFromId(clientTypeId);
		}
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static  ClientTypeInfo Find(List< ClientTypeInfo> list,  int clientTypeId)
        {
             return list.Find(delegate( ClientTypeInfo item) { return item. clientTypeId==clientTypeId; });
        }
        ...
        {
            return string.Format(CachedEntityCommander.CacheKey, typeof( ClientTypeInfo).ToString());
        }
		...
		public  override void Delete()
		{
			if(!m_Loaded)
				throw new AppException("尚未初始化");
			bool result=  (ClientType.Delete(ClientTypeId) == 1);
			//更新缓存
			if (result&&CachedEntityCommander.IsTypeRegistered(typeof(ClientTypeInfo))) 
            {
                ResetCache();
            }
            if(!result)
		...
		public override void Save()
		{
			...
			else//修改
			{
				...
				SaveToDb(this, clientType,false);
			}
