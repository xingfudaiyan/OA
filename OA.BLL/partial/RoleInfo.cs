...
namespace CanYou.OA.BLL
{
	...
	/// </summary>
	[Serializable]
	public partial class RoleInfo:CanYou.EntityObject
	{
		#region 属性
		///<summary>
		///
		...
		///
		///</summary>
		public RoleInfo(int pRoleId,string pRoleName)
		{
			roleId   = pRoleId;
			roleName = pRoleName;
		...
		private void LoadFromId(int roleId)
		{
		 
			if (CachedEntityCommander.IsTypeRegistered(typeof(RoleInfo)))
            {
				RoleInfo roleInfo=Find(GetList(), roleId);
				if(roleInfo==null)
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static  RoleInfo Find(List< RoleInfo> list,  int roleId)
        {
             return list.Find(delegate( RoleInfo item) { return item. roleId==roleId; });
        }
        ...
        {
            return string.Format(CachedEntityCommander.CacheKey, typeof( RoleInfo).ToString());
        }
        ...
        public static void Copy(RoleInfo pRoleInfoFrom, RoleInfo pRoleInfoTo)
        {
	 		pRoleInfoTo.RoleId = pRoleInfoFrom.roleId;
	 		pRoleInfoTo.RoleName = pRoleInfoFrom.roleName;
			pRoleInfoTo.Loaded=pRoleInfoFrom.Loaded;
        }
		...
		//从后台获取数据
		internal  static void  LoadFromDAL(RoleInfo pRoleInfo, Role  pRole)
		{
	 		pRoleInfo.roleId = pRole.RoleId;
	 		pRoleInfo.roleName = pRole.RoleName;
			pRoleInfo.Loaded=true;
		...
		public  override void Delete()
		{
			if(!m_Loaded)
				throw new AppException("尚未初始化");
			bool result=  (Role.Delete(RoleId) == 1);
			//更新缓存
			if (result&&CachedEntityCommander.IsTypeRegistered(typeof(RoleInfo))) 
            {
                ResetCache();
            }
            if(!result)
        ...
        /// </summary>
        /// <param name="pIndustryTypeInfoTo"></param>
        public void CopyTo(RoleInfo pRoleInfoTo)
        {
            Copy(this,  pRoleInfoTo);
        }
