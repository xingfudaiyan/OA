...
namespace CanYou.OA.BLL
{
	...
	/// </summary>
	[Serializable]
	public partial class PoliticsInfo:CanYou.EntityObject
	{
		#region 属性
		///<summary>
		///政治面貌Id
		...
		///政治面貌
		///</summary>
		private string politicsName = String.Empty;
		...
		public struct Fields
		{
			...
			public const string PoliticsName=@"PoliticsName";
		...
		///
		///</summary>
		public PoliticsInfo(int pPoliticsId,string pPoliticsName)
		{
			politicsId   = pPoliticsId;
			politicsName = pPoliticsName;
		...
		///
		///</summary>
		public PoliticsInfo(int politicsId)
		{
			 LoadFromId(politicsId);
		}
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static  PoliticsInfo Find(List< PoliticsInfo> list,  int politicsId)
        {
             return list.Find(delegate( PoliticsInfo item) { return item. politicsId==politicsId; });
        }
        ...
        {
            return string.Format(CachedEntityCommander.CacheKey, typeof( PoliticsInfo).ToString());
        }
        ...
        public static void Copy(PoliticsInfo pPoliticsInfoFrom, PoliticsInfo pPoliticsInfoTo)
        {
	 		pPoliticsInfoTo.PoliticsId = pPoliticsInfoFrom.politicsId;
	 		pPoliticsInfoTo.PoliticsName = pPoliticsInfoFrom.politicsName;
			pPoliticsInfoTo.Loaded=pPoliticsInfoFrom.Loaded;
        }
		
		/// <summary>
		/// 批量装载
		/// </summary>
		internal static void LoadFromDALPatch(List< PoliticsInfo> pList, PoliticsCollection pCollection)
        {
            foreach (Politics politics in pCollection)
            {
                PoliticsInfo politicsInfo = new PoliticsInfo();
                LoadFromDAL(politicsInfo, politics );
                pList.Add(politicsInfo);
            }
		...
		/// </summary>
		/// <returns></returns>
		public static List< PoliticsInfo> GetList()
		{
			string cacheKey = GetCacheKey();
			//本实体已经注册成缓存实体，并且缓存存在的时候，直接从缓存取
			if (CachedEntityCommander.IsTypeRegistered(typeof(PoliticsInfo)) && CachedEntityCommander.GetCache(cacheKey) != null)
            {
                return CachedEntityCommander.GetCache(cacheKey) as List< PoliticsInfo>;
            }
            else 
            {
				List< PoliticsInfo>  list =new List< PoliticsInfo>();
				PoliticsCollection  collection=new  PoliticsCollection();
				Query qry = new Query(Politics.Schema);
        ...
        /// </summary>
        /// <param name="pIndustryTypeInfoTo"></param>
        public void CopyTo(PoliticsInfo pPoliticsInfoTo)
        {
            Copy(this,  pPoliticsInfoTo);
        }
