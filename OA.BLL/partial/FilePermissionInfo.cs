...
{
	/// <summary>
	///FilePermissionInfo  Data
	/// </summary>
	[Serializable]
	public partial class FilePermissionInfo:CanYou.EntityObject
	{
		#region 属性
		///<summary>
		///
		...
		///
		///</summary>
		private string filePermissionName = String.Empty;
		...
		public struct Fields
		{
			...
			public const string FilePermissionName=@"FilePermissionName";
		...
		///
		///</summary>
		public FilePermissionInfo(int pFilePermissionId,string pFilePermissionName)
		{
			filePermissionId   = pFilePermissionId;
			filePermissionName = pFilePermissionName;
		...
		///
		///</summary>
		public FilePermissionInfo(int filePermissionId)
		{
			 LoadFromId(filePermissionId);
		}
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static  FilePermissionInfo Find(List< FilePermissionInfo> list,  int filePermissionId)
        {
             return list.Find(delegate( FilePermissionInfo item) { return item. filePermissionId==filePermissionId; });
        }
        ...
        {
            return string.Format(CachedEntityCommander.CacheKey, typeof( FilePermissionInfo).ToString());
        }
        ...
        public static void Copy(FilePermissionInfo pFilePermissionInfoFrom, FilePermissionInfo pFilePermissionInfoTo)
        {
	 		pFilePermissionInfoTo.FilePermissionId = pFilePermissionInfoFrom.filePermissionId;
	 		pFilePermissionInfoTo.FilePermissionName = pFilePermissionInfoFrom.filePermissionName;
			pFilePermissionInfoTo.Loaded=pFilePermissionInfoFrom.Loaded;
        }
		
		/// <summary>
		/// 批量装载
		/// </summary>
		internal static void LoadFromDALPatch(List< FilePermissionInfo> pList, FilePermissionCollection pCollection)
        {
            foreach (FilePermission filePermission in pCollection)
            {
                FilePermissionInfo filePermissionInfo = new FilePermissionInfo();
                LoadFromDAL(filePermissionInfo, filePermission );
                pList.Add(filePermissionInfo);
            }
		...
		/// </summary>
		/// <returns></returns>
		public static List< FilePermissionInfo> GetList()
		{
			string cacheKey = GetCacheKey();
			//本实体已经注册成缓存实体，并且缓存存在的时候，直接从缓存取
			if (CachedEntityCommander.IsTypeRegistered(typeof(FilePermissionInfo)) && CachedEntityCommander.GetCache(cacheKey) != null)
            {
                return CachedEntityCommander.GetCache(cacheKey) as List< FilePermissionInfo>;
            }
            else 
            {
				List< FilePermissionInfo>  list =new List< FilePermissionInfo>();
				FilePermissionCollection  collection=new  FilePermissionCollection();
				Query qry = new Query(FilePermission.Schema);
		...
		{
			get {return filePermissionName;}
			set {filePermissionName = value;}
		}
        ...
        /// </summary>
        /// <param name="pIndustryTypeInfoTo"></param>
        public void CopyTo(FilePermissionInfo pFilePermissionInfoTo)
        {
            Copy(this,  pFilePermissionInfoTo);
        }
